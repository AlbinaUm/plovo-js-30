...
const cartSlice = createSlice({
   ...
   reducers: {
       ...
       updateDishesInCart: (state, action: PayloadAction<IDish[]>) => {
           const dishes = action.payload; // эти блюда не из корзины, они летят с Firebase
           const newCartDishes: CartDish[] = [];

           // блюда из Firebase сверяются с блюдами из корзины для того чтобы актулизировать данные в корзине
